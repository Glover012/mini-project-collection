CREATE OR REPLACE FUNCTION get_cars_by_ids3( ids INT[] )
RETURNS TABLE(
	car_id INT,
	car_top_speed NUMERIC) 
AS $$
DECLARE
	current_id INT;
	max_id INT;
BEGIN
	-- Stores the maximum id from cars in the local max_id variable.
	SELECT MAX(id) INTO max_id FROM cars;

	FOREACH current_id IN ARRAY ids
	LOOP
		EXIT WHEN current_id <= 0; -- If the id is less than or equal to zero, exits the loop. Works like break.

		IF current_id > max_id THEN 
			EXIT; -- If the id is outside the range, exits the loop.
		END IF;

		RETURN QUERY SELECT
			id,
			top_speed
		FROM cars WHERE id = current_id;
	END LOOP;
END;
$$ LANGUAGE plpgsql;


SELECT * FROM get_cars_by_ids3( ARRAY[-1,1,1000,3,5,7] );
